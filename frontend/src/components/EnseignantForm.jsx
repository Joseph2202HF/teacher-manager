...
export default function EnseignantForm({ mode = 'create' }) {
  ...
  const salaireInfo = (() => {
    ...
    if (isNaN(heures) || isNaN(taux) || heures < 0 || taux < 0) return null
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true)
    const payload = { nom: form.nom.trim(), nbheures: parseFloat(form.nbheures), tauxhoraire: parseFloat(form.tauxhoraire) }
  ...
  const getInputStyle = (fieldName) => {
    ...
    if (!isDirty) {
      return `${base} input-dark`
    }
    if (error) {
      return `${base} bg-rose-500/5 border-2 border-rose-500/50 text-[var(--text-primary)] focus:ring-2 focus:ring-rose-500/20 placeholder:text-rose-300/30`
    }
          ...
          <form onSubmit={handleSubmit} noValidate className="space-y-6">
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-sm font-mono font-bold" style={{ 
                    color: progressionPercent === 100 ? '#059669' : '#06b6d4'
                  }}>
                    {progressionPercent}%
                  </span>
                </div>
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 text-sm">
                        <span className="px-2 py-1 rounded-lg font-mono text-cyan-400" style={{ background: 'var(--surface-bg)' }}>
            ...
            <div className="flex items-center gap-3">
              <button
                type="submit"
                disabled={loading}
                className={`btn-primary flex-1 ${!formValid ? 'opacity-60' : ''}`}
              >
                {loading ? (
                  <>
            ...
            {/* ═══ Messages de statut ═══ */}
            {formValid && (
              <div className="flex items-start gap-3 px-5 py-3.5 rounded-xl" style={{
